...
class RandomScale(object):
    # This class is modified to align with the logic from CFDHI-Net's dataloader.
    def __init__(self, scales):
        # Expects a list of discrete scale factors, not a range.
    ...
    def __call__(self, sample):
        ...
        # Choose a random scale from the provided list, same as CFDHI-Net.
        scale = random.choice(self.scales)
        size = (int(round(h * scale)), int(round(w * scale)))

        for key in sample.keys():
            # Aligning interpolation methods with CFDHI-Net:
            # BILINEAR for image and depth (modal_x).
            # NEAREST for label, bound, edge, etc.
