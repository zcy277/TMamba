...
class IRSeg(data.Dataset):
    ...
    def __init__(self, cfg, root, mode='trainval', do_aug=True):
        ...
        # scale_range = tuple(float(i) for i in cfg['scales_range'].split(' ')) # Old: continuous scale range
        ...
        self.aug = Compose([
            # ColorJitter is removed to align with CFDHI-Net's augmentation strategy.
            # ColorJitter(
            #     brightness=cfg.get('brightness', 0), # Use .get for safety if keys are commented out
            #     contrast=cfg.get('contrast', 0),
            #     saturation=cfg.get('saturation', 0)),
            RandomHorizontalFlip(cfg['p']),
            RandomScale(scales_array), # Use the new discrete scales array with the updated RandomScale class
